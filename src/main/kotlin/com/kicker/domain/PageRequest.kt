...
class PageRequest(
    ...
    override fun getSort(): Sort = Sort(sortDirection, "id")

    override fun next(): Pageable = PageRequest(offset + limit, limit, sortDirection)

    override fun getPageSize(): Int = limit

    override fun getOffset(): Long = offset.toLong()

    override fun first(): Pageable = PageRequest(0, limit, sortDirection)

    private fun previous(): Pageable {
        if (hasPrevious()) {
            var newOffset = this.offset - limit
            if (newOffset < 0) newOffset = 0
            return PageRequest(newOffset, limit, sortDirection)
        }
        return first()
    }
